class Library {
        ...
        try {
            if (this.isString(clientName) && this.isString(book)) {
                ...
                if (wantedBook) {
                    ...
                    this.dates.push({title: wantedBook[0].title, borrowDate: Date.now()});
    ...
    async returnBook (clientName, book) {
        try {
            if (this.isString(clientName) && this.isString(book)) {
                const booktitle = book.toLowerCase();
                const checkBorrow = this.dates.find(book => book.title === booktitle);
                ...
                if (checkBorrow) {
                    ...
                    if (index >= 0) {
                        ...
                        throw new Error("could't find your data");
                    }
                } else {
